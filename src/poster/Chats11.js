...
// import { WebSocketDemo } from './sockect/WebSocketDemo';
...
const Chats = () => {
  ...
  const socketUrl = 'ws://170.64.131.134:5000';
  ...
  const { sendJsonMessage, readyState } = useWebSocket(socketUrl, {
    ...
    onMessage: (event) => {
      ...
      if (info.sender_id === newMessage.reciever_id && info.reciever_id === newMessage.sender_id && info.job_id === newMessage.job_id) {
        const updateCount = { roomid: newMessage.roomId, job_id: newMessage.job_id };
  ...
  const handleButtonClick = (item) => {
    ...
    if (item.roomdata.messageCount > 0) {
      updateMessageCount(item.roomdata._id, jobid);
    }
    setUser(item);
  ...
  const updateMessageCount = async (roomid, jobid) => {
    try {
      const param = { roomid: roomid, job_id: jobid };
      const res = await axiosInstance.post('/user/updateMessageCount', param);
      if (res.data.status == 200) {
        setFilterList(res?.data?.getChatUserList);
      }
    } catch (error) {
      console.log(error);
    }
                ...
                <Grid item xl="3" lg="4" md="12">
                  <div className="chat_listing">
                    <div className="serch_listing d-flex">
                      <input type="text" placeholder="Search" value={searchVal.user} name="user" onChange={(e) => handleSearch(e)} />
                      {btnLoading ? (
                          ...
                          >
                            <div className="user_chat">
                              <div className="user_profile">
                                ...
                                {item.roomdata.messageCount != 0 && <p> {item.roomdata.messageCount}</p>}
                              </div>
                            </div>
                          </button>
                        ))
